    ...
    questions: [
      {
        ...
        workedSolution:
`**Introduction.** Port congestion and logistics failures flow straight through to retailers like Woolworths, threatening the operations objectives of speed, dependability and cost (Slack, Jones and Burgess, 2022).

**Analysis of impacts on Woolworths.** Delays at the port cause *stockouts* of imported lines and *late delivery* to stores, damaging dependability and the customer promise. They tie up *working capital* in inventory stranded at the port and raise *landed cost* through demurrage, storage and expedited freight, hurting the cost objective. Perishable and fashion lines are worst hit because they are time-sensitive, so delays cause markdowns and waste. The cumulative effect is weaker service, higher cost and reputational damage in a premium-positioned retailer where availability is part of the brand.

**Mitigation strategies, with examples.** (1) *Supplier diversification* — dual-source critical lines across regions so one port problem is not fatal. (2) *Safety stock on critical SKUs* — hold buffer inventory for high-impact, time-sensitive items. (3) *Alternative ports and modes* — route through different ports or shift urgent goods to air freight. (4) *Nearshoring/local sourcing* — source more locally to shorten and de-risk lead times. (5) *Collaborative forecasting and longer buffer lead times* — plan with suppliers and build realistic lead times into ordering.

**Conclusion.** Woolworths should combine inventory buffers on critical lines with structural changes (diversified, more local sourcing and flexible logistics) so that port volatility degrades cost and service less severely (Slack, Jones and Burgess, 2022).`,
        answerPlan: [
      ...
      },
      {
        ...
        workedSolution:
`**Introduction.** Improving Transnet's port and rail capability is an operations problem spanning capacity, process and supply-chain management (Slack, Jones and Burgess, 2022; Heizer, Render and Munson, 2020).

**Diagnosing the capability gap.** The core constraints are ageing equipment, poor maintenance reliability, capacity bottlenecks at berths and weak coordination with carriers, which together cause congestion and unreliable throughput.

**Capacity strategies.** Renew and maintain critical equipment (cranes, locomotives) on a planned preventive-maintenance regime to raise availability; expand and debottleneck berth and rail capacity at the binding constraint rather than across the board; and use the theory-of-constraints logic of lifting the bottleneck first.

**Process strategies.** Improve scheduling and berth allocation to raise throughput; apply lean and TQM principles to reduce variability and rework; and measure utilisation and efficiency so improvement is evidence-based.

**Supply-chain strategies.** Collaborate with shipping lines, rail customers and terminal operators; set and publish clear performance KPIs (dwell time, turnaround); and use private-sector partnerships and concessions to bring in capital and operational expertise.

**Conclusion.** The measures I would present are: a preventive-maintenance and equipment-renewal programme to restore reliability, targeted capacity expansion at bottlenecks, lean process and scheduling improvements, and stronger supply-chain collaboration with carriers and private partners. Sequenced this way, reliability is restored first, then capacity and coordination scale throughput (Slack, Jones and Burgess, 2022).`,
        answerPlan: [
      ...
      },
      {
        ...
        workedSolution:
`**Introduction.** A facility-location decision should weigh a structured set of factors; the factor-rating method gives a disciplined way to compare Lesotho and Eswatini (Heizer, Render and Munson, 2020).

**Location factors, applied to Lesotho versus Eswatini.** (1) *Labour cost and availability* — wage levels, skills and labour stability in each country. (2) *Proximity to markets* — distance and access to the Southern African target markets the plant will serve. (3) *Proximity to suppliers and raw materials* — closeness to inputs reduces inbound transport cost and lead time. (4) *Infrastructure and utilities* — reliable electricity, water and telecommunications, critical for food processing. (5) *Transport and logistics* — road and rail links and access to ports for any imports/exports. (6) *Political and economic stability, regulation and incentives* — tax regimes, trade agreements (for example membership of SACU), investment incentives and currency. (7) *Exchange rate and cost of doing business.*

**Method.** Using factor rating, Nestlé would (a) list these factors, (b) weight each by importance, (c) score Lesotho and Eswatini on each, and (d) compute weighted scores; the higher total indicates the preferred site, while making the trade-offs explicit.

**Conclusion.** Rather than choosing intuitively, Nestlé should rate both countries against weighted location factors, so the decision reflects market access, input proximity, infrastructure reliability and stability in a transparent, defensible way (Heizer, Render and Munson, 2020).`,
        answerPlan: [
...
**3.2 Coke Zero.** Labour productivity = 5 gallons per 4,000 hours; with 100,000 labour hours available: gallons = (5 \u00f7 4,000) \u00d7 100,000 = **125 gallons** for the quarter. If multifactor productivity = 1 gallon per R25,000, then spend to produce 125 gallons = 125 \u00d7 R25,000 = **R3,125,000**.`,
      ...
      },
      {
        ...
        workedSolution:
`**Introduction.** Trend (linear regression) projection fits a straight line to historical data and extends it to forecast future periods; the line is y = a + bx (Heizer, Render and Munson, 2020).

**Method (worked with the actual 2023 monthly figures from the paper).** Code the twelve 2023 months as x = 1...12 and let y be each month's sales. Then compute:
- b (slope) = (Σxy − n·x̄·ȳ) ÷ (Σx² − n·x̄²)
- a (intercept) = ȳ − b·x̄, where x̄ = 6.5 and Σx² for 1...12 = 650.

Build a working table with columns x, y, xy and x², total the columns, and substitute into the formulas above to obtain a and b. (Illustrative: if the totals gave, for example, a = 100 and b = 5, the fitted line would be y = 100 + 5x.)

**Projection for Jan-Apr 2024.** Continue the x sequence: January 2024 = x 13, February = 14, March = 15, April = 16. Substitute each into y = a + bx to get the four forecasts (e.g. with the illustrative line: 165, 170, 175, 180).

**Interpretation.** Trend projection assumes the historical pattern continues; it captures direction but not seasonality, so for seasonal products it should be combined with seasonal indices.

**Conclusion.** Tabulate x, y, xy, x²; compute b then a; and project x = 13-16 for the four 2024 months, stating the assumption that the trend persists (Heizer, Render and Munson, 2020).`,
        answerPlan: [
...
**Conclusion.** For a fashion retailer such as TFG, flexibility and speed are order-winners because of short product cycles, while quality and dependability are order-qualifiers, and all four converge on lower cost (Slack, Jones and Burgess, 2022).`,
      ...
      },
      {
        ...
        recommendedTheories: ['om-performance-objectives'],
        workedSolution:
`**Introduction.** Operations strategy can be read through the five performance objectives: quality, speed, dependability, flexibility and cost; TFG used these to drive improvement (Slack, Jones and Burgess, 2022).

**Mapping case evidence to each objective.** (1) *Speed* — order-to-delivery time was cut dramatically (from roughly 17 days to under 5), so customers are served faster. (2) *Dependability* — delivery conformance improved markedly (around an eleven-percentage-point gain), meaning more orders arrive when promised. (3) *Quality* — picking accuracy reached near-perfect levels (about 99.9%), reducing errors and returns. (4) *Flexibility* — a standard template was rolled out across formats, so the operation can adapt and scale to different store types. (5) *Cost* — faster, more accurate, more reliable flow lowers waste, rework and working capital, improving cost performance.

**Critical assessment.** The gains are real and mutually reinforcing: better quality and dependability reduce cost, and speed improves customer service. But they depend on partner capability and disciplined, phased execution, and create a degree of dependency on the systems and partners that delivered them; sustaining the gains requires continued investment and oversight.

**Conclusion.** TFG turned each performance objective into a measurable improvement, and the combined effect was a faster, more reliable, higher-quality and lower-cost operation; the strategic payoff is stronger competitiveness, provided the underlying capability is maintained (Slack, Jones and Burgess, 2022).`,
        answerPlan: [
...
**3.4 Evaluate the EOQ assumptions.** EOQ assumes constant/known demand, constant lead time, a fixed unit price (no quantity discounts), the entire order arriving at once, and only ordering and holding costs mattering (Heizer, Render and Munson, 2020). In reality demand and lead time vary, suppliers offer discounts, and stockout costs exist, so EOQ is a useful baseline rather than an exact optimum and should be adjusted with safety stock and discount models.`,
      ...
      },
      {
        ...
        unitIds: ['om-4'], theoryIds: ['om-forecasting'],
        workedSolution:
`**Introduction.** A weighted moving average (WMA) forecasts demand by weighting recent periods more heavily than older ones; forecast error is then summarised by the mean absolute deviation (Heizer, Render and Munson, 2020).

**4.1 WMA method and deviations.** State the formula: WMA = Σ(weight × demand in that period) ÷ Σ(weights). Where the paper does not specify weights, assume weights of 3, 2, 1 for the most recent, middle and oldest of the three months (stating this assumption explicitly). For each forecastable month from month 4 onward, take the previous three actuals, apply the weights, and divide by 6 to get the forecast. Then compute the deviation for each month as deviation = actual − forecast. (Illustrative: actuals 100, 110, 120 give a forecast for month 4 of (3×120 + 2×110 + 1×100) ÷ 6 = 680 ÷ 6 ≈ 113.3.)

**4.2 Mean absolute deviation.** MAD = Σ|actual − forecast| ÷ n, where n is the number of forecast periods. Take the absolute value of each deviation, sum them and divide by the count.

**Interpretation.** The MAD expresses the average size of forecast error in units; a smaller MAD means a more accurate forecast, and it can be compared across methods to choose the best one.

**Conclusion.** State the weights, compute each month's WMA and deviation in a table, then average the absolute deviations for the MAD, remembering that omitting the absolute value is the most common error (Heizer, Render and Munson, 2020).`,
        answerPlan: [
      ...
      },
      {
        ...
        workedSolution:
`**Introduction.** An international purchase of ostrich eggs from Oudtshoorn (South Africa) by AVO (France) raises the standard problems of cross-border procurement (Heizer, Render and Munson, 2020; Slack, Jones and Burgess, 2022).

**International purchasing problems, applied to the transaction.** (1) *Currency and exchange-rate risk* — the deal spans rand and euro, so movements between order and payment change the real cost; this is relevant to both the SA exporter's revenue and the French importer's cost. (2) *Longer and more uncertain lead times* — distance and shipping make replenishment slow and variable, a serious issue for perishable eggs. (3) *Customs, tariffs and export/import documentation* — clearance, duties and paperwork add cost and delay at both ends. (4) *Quality and specification assurance* — verifying quality and condition across borders is harder, and perishability raises spoilage risk in transit. (5) *Communication, culture and legal-jurisdiction differences* — language, business practice and which country's law governs disputes. (6) *Logistics and cold-chain reliability* for a fragile, perishable product.

**Mitigations.** Use forward contracts or hedging for currency risk; agree clear *Incoterms* to allocate transport risk and cost; build quality agreements and inspection into the contract; and plan cold-chain logistics with buffer lead times.

**Conclusion.** These problems are highly relevant here because the product is perishable and the transaction crosses currencies, customs regimes and a long supply line; clear contracts, Incoterms, hedging and cold-chain control are essential (Slack, Jones and Burgess, 2022).`,
        answerPlan: [
      ...
      },
      {
        ...
        workedSolution:
`**Introduction.** Operations differ along four dimensions, the four Vs: volume, variety, variation and visibility, which together shape cost and process design (Slack, Jones and Burgess, 2022).

**The four Vs (with a simple spectrum diagram).** Picture each V as a low-to-high spectrum:
- Volume:        low |-----------X-----| high
- Variety:       low |---X-------------| high
- Variation:     low |------X----------| high
- Visibility:    low |--------X--------| high

(1) *Volume* — how much is produced; high volume enables repetition, specialisation and low unit cost. (2) *Variety* — the range of products/services; high variety needs flexibility but raises cost. (3) *Variation in demand* — how much demand fluctuates; high variation needs spare capacity and raises cost. (4) *Visibility* — how much of the operation the customer sees; high visibility needs strong customer-contact skills and short tolerance for delay.

**Application and evaluation (chosen organisation, e.g. a fast-food chain).** A fast-food outlet is high volume, low variety, moderate variation and high visibility: this profile drives standardised processes, low unit cost and fast service, but limits customisation. *Interrelationships:* the Vs interact — high volume with low variety supports low cost, whereas adding variety or visibility raises cost and complexity. *Operational challenges:* managing demand variation (peaks), and balancing customer-facing visibility with back-office efficiency.

**Conclusion.** The four-V profile explains why this operation is designed for low-cost, fast, standardised delivery, and where its cost and flexibility tensions lie (Slack, Jones and Burgess, 2022).`,
        answerPlan: [
    ...
    questions: [
      {
        ...
        workedSolution:
`**Introduction.** Comparative and competitive advantage explain why location matters to operations; Porter's diamond shows that national/locational conditions shape a firm's competitiveness, which is central to Apple's relocation decision (Heizer, Render and Munson, 2020).

**Comparative advantage and Porter's diamond.** Comparative advantage is a location's ability to produce something relatively more efficiently. Porter's diamond identifies four locational determinants: *factor conditions* (skilled labour, infrastructure, inputs), *related and supporting industries* (supplier clusters), *demand conditions* (sophisticated local demand) and *firm strategy, structure and rivalry*.

**Application to Apple's relocation.** Apple's current Asian production benefits from a dense supplier cluster, deep manufacturing skills and scale that no single new location replicates quickly. Relocating risks losing access to that *cluster* of related and supporting industries, raising factor costs (higher wages, scarcer specialised labour) and lengthening or breaking established supply links. Against this, the new location may offer political stability, demand or strategic benefits.

**Analysis of the net effect on operational advantage.** On the operations objectives, relocation likely raises *cost*, slows *speed* while the supply base rebuilds, and reduces *flexibility* until a local ecosystem matures. The lost cluster economies are the decisive factor.

**Conclusion.** Applying Porter's logic, relocating away from an established cluster is likely to erode Apple's operational competitive advantage in the short to medium term, unless a comparable supporting-industry ecosystem can be built at the new location (Heizer, Render and Munson, 2020).`,
        answerPlan: [
...
- (2.6) Total cost (ordering + carrying) \u2248 950 + 948 = **R1,898/year**.`,
      ...
      },
      {
        ...
        workedSolution:
`**Introduction.** The product life cycle (PLC) traces a product through introduction, growth, maturity and decline, and the operations focus shifts at each stage (Heizer, Render and Munson, 2020).

**The four stages, with operations focus (chosen product, e.g. a smartphone model).** (1) *Introduction* — low sales, high cost, frequent design changes; operations emphasise *product design flexibility*, low-volume production and getting quality right, with high unit cost. (2) *Growth* — sales rise and competitors enter; operations shift to *increasing capacity*, improving forecasting and stabilising the process to meet demand while still adding variants. (3) *Maturity* — sales peak and competition is intense; operations focus on *cost reduction, efficiency, standardisation and high-volume, low-cost production*, squeezing waste and improving yield. (4) *Decline* — sales fall; operations focus on *cost control, capacity reduction, rationalising the range* and deciding whether to harvest or withdraw.

**Operations implication.** The emphasis moves from *design and flexibility* early in the cycle to *efficiency and cost* later, so process choice and capacity decisions must evolve with the product.

**Conclusion.** Reading the chosen product across the PLC shows operations re-balancing from flexible, design-led production at launch to efficient, cost-led production at maturity, and disciplined rationalisation in decline (Heizer, Render and Munson, 2020).`,
        answerPlan: [
...
**4.3 Critical path** = the longest path = **B\u2192E\u2192F\u2192H\u2192I, duration 64 days**. (Draw the AON with nodes for each activity; arrows show precedence. F has two predecessors, C and E; I has two predecessors, G and H.)`,
      ...
      },
      {
        ...
        unitIds: ['om-10'], theoryIds: ['om-inventory'],
        workedSolution:
`**Introduction.** ABC analysis applies the Pareto principle to inventory, classifying items by their share of annual demand value so control effort is focused where it matters (Heizer, Render and Munson, 2020).

**Method (to apply to the ten stock units in the paper).** (1) For each item, compute annual demand value (annual demand × unit cost where given, or average demand as specified). (2) Rank the items from highest to lowest value. (3) Compute each item's percentage of total value and the running cumulative percentage. (4) Classify by the stated cut-offs: items making up roughly the top 80% of value = *A*, the next ~15% = *B*, and the final ~5% = *C*. Typically a few A-items dominate value while many C-items contribute little.

**Illustrative table (format to complete with the paper's figures).**
- Item, Annual value, % of total, Cumulative %, Class
- Highest-value items accumulate to ~80% -> A
- Next band to ~95% -> B
- Remainder to 100% -> C

**Control policy per class.** *A-items* — tight control, frequent review, accurate records, close supplier management. *B-items* — moderate control and periodic review. *C-items* — simple control, larger order quantities, minimal monitoring.

**Conclusion.** Rank by value, cumulate the percentages, classify A/B/C at 80/15/5, and apply progressively lighter control from A to C, concentrating management attention on the high-value A-items (Heizer, Render and Munson, 2020).`,
        answerPlan: [
